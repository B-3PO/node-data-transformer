...
function buildQueries(struct, obj) {
  ...
  // add struct join
  if (obj.root !== struct && obj.joinedResources.indexOf(struct.$$resource.name) === -1) { // is not root resource
    ...
    relations.forEach(function (rel) {
      ...
      if (obj.joinedResources.indexOf(rel.root.name) !== -1) { return; }

      obj.joins.push(buildRelationJoin(rel));
      obj.joinedResources.push(rel.root.name);
    });
  }
