...
function modefiers(dataPromise, formatter) {
  return {
    ...
    wrap: wrap(dataPromise, formatter),
...
function wrap(dataPromise, formatter) {
  return function (fn) {
    dataPromise.then(function (dataObj) {
      if (typeof fn === 'function') { dataObj.root = fn(dataObj.root); }
      else { console.error('wrap requires a function that returnes a object'); }
      return dataObj;
    }).catch(function (err) { console.error(err.stack) });
    return formatter.extend(modefiers(dataPromise, formatter));
  };
