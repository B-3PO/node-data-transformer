...
exports.build = function getQuery(ids, queryFilters) {
  ...
  return (split) => {
    ...
    // queryObj.query += addWhereFilters(struct.$$filters, queryObj.joinedResources, hasWhere);
...
function buildRelationJoin(rel, queryFilters) {
  var join = `LEFT JOIN ${rel.resource.table} ON ${rel.resource.table}.${rel.resourceField} = ${rel.root.table}.${rel.rootField}`;
  (queryFilters[rel.resource.type] || []).forEach(item => {
    var resource = getResource(rel.resource.type);
    Object.keys(item).forEach(key => {
      let prefix = 'AND';
      let operator = '=';
      if (item[key] === null) { operator = 'IS'; }
      join += ` ${prefix} ${resource.table}.${key} ${operator} ${dataTypes.parse[resource.fields[key].dataType](item[key])}`;
    });
  });
  return join;
}
