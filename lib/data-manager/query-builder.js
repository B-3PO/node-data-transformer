...
function addWhereFilters(filters, joinedResources, hasWhere) {
  return Object.keys(filters)
    .filter(key => joinedResources.indexOf(key) > -1)
    .map(key => { return { arr: filters[key], type: key }; })
    .reduce((a, b) => {
      var resource = getResource(b.type);
      b.arr.forEach(obj => {
        Object.keys(obj).forEach(key => {
          ...
          a += ` ${prefix} ${resource.table}.${key} ${operator} ${dataTypes.parse[resource.fields[key].dataType](obj[key])}`;
        });
      });
      return a;
    }, '');
...
function buildRelationJoin(rel) {
  return `LEFT JOIN ${rel.resource.table} ON ${rel.resource.table}.${rel.resourceField} = ${rel.root.table}.${rel.rootField}`;
}
