...
const getResource = require('../resource-manager').get;

exports.build = function getQuery(ids, queryFilters) {
  ids = [].concat(ids || []);
  return (split) => {
    const struct = [].concat(split.structs)[0];
    const queryObj = buildQuery(struct, createQueryObj(struct), split, queryFilters);
    ...
    if (queryFilters[queryObj.root.$$resource.type]) {
      if (hasWhere) {
        queryObj.query += buildQueryFilter(queryFilters, queryObj.root.$$resource.type);
      } else {
        queryObj.query += buildQueryFilter(queryFilters, queryObj.root.$$resource.type, true);
      }
    }
    if (queryLog.active) { queryLog.sql(queryObj.query); }
    return queryObj;
  };
};


function buildQuery(struct, obj, split, queryFilters) {
  addAttributes(struct, obj);
  addStructJoin(struct, obj, queryFilters);
  addFieldJoins(struct, obj, queryFilters);
  ...
  Object.keys(struct).forEach(function (key) {
    ...
    buildQuery(struct[key], obj, split, queryFilters);
  });
  return obj;
}
...
function addStructJoin(struct, obj, queryFilters) {
...
function buildRelationJoin(rel, queryFilters) {
  return `LEFT JOIN ${rel.resource.table} ON ${rel.resource.table}.${rel.resourceField} = ${rel.root.table}.${rel.rootField}${buildQueryFilter(queryFilters, rel.resource.type)}`;
}

function buildQueryFilter(queryFilters, type, needsWhere) {
  var str = '';
  (queryFilters[type] || []).forEach(item => {
    ...
    Object.keys(item).forEach(key => {
      ...
      let operator = '=';
      if (item[key] === null) { operator = 'IS'; }
      var value = dataTypes.parse[resource.fields[key].dataType](item[key]);
      value = typeof value === 'string' ? `\'${value}\'` : value;
      str += ` ${prefix} ${resource.table}.${key} ${operator} ${value}`;
    });
  });
  return str;
}
...
function getAttrQuery(queryObj) {
  // filter out attrs that are not directly hooked up to a field
  return Object.keys(queryObj.attrs).filter(key => {
    return !queryObj.attrs[key].config.concat;
  }).map(key => {
