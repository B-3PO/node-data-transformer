...
function Constructor(struct, single) {
  ...
  function filter(type, obj) {
    if (typeof type !== 'string') {
      throw Error('filters first param must be an type string');
    }
    if (!(obj instanceof Object)) {
      throw Error('filter second param must be an object');
    }

    // TODO fix this. the stuct it is refernceing is not correct. need to getStruct
    // var props = Object.keys(struct);
    // if (Object.keys(obj).filter(key => props.indexOf(key) > -1).length > 0) {
    //   throw Error('invalid filter properties');
    // }
    if (!struct.$$filters[type]) { struct.$$filters[type] = []; }
    struct.$$filters[type].push(obj);
    return struct;
  }
}
